...
if(isset($_GET['id'])){
  try{
    ...
    //idパラメータの値を同じidのデータが存在しない場合はエラーメッセージを表示して処理を終了する
    if($book===FALSE){
      exit('idパラメータの値が不正です。');
    }
    //genreテーブルからgenre_codeカラムのデータを取得するためのSQL文を変数$sql_select_codesに代入
  ...
  //idパラメータの値が存在しない場合エラーメッセージを表示して処理を停止する
    ...
    <article class="registration">
      ...
      <form action="update.php?id=<?= $_GET['id'] ?>" method="post" class="registration-form">
        <div>
          ...
          <input type="number" id="book_code" name="book_code" value="<?= $book['book_code'] ?>" min="0" max="1000000" required>

          <label for="book_name">書籍名</label>
          <input type="text" id="book_name" name="book_name" value="<?= $book['book_name'] ?>" maxlength="50" required>

          <label for="price">単価</label>
          <input type="number" id="price" name="price" value="<?= $book['price'] ?>" min="0" max="1000000" required>

          <label for="stock_quantity">在庫数</label>
          <input type="number" id="stock_quantity" name="stock_quantity"  value="<?= $book['stock_quantity'] ?>"min="0" max="1000000" required>

          <label for="genre_code">ジャンルコード</label>
          <select id="genre_code" name="genre_code" required>
            <option disabled selected value>選択してください</option>
            <?PHP
            //配列の中身を順番に取り出し、セレクトボックスの選択肢としてしゅつりょくする
            foreach($genre_codes as $genre_code){
              //もし偏す$genre_codが商品仕入れ先コードのあたいを一致していれば、selected属性をつけて初期値にする
