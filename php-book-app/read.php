...
try{
 ...
 //orderパラメータの値が存在すれば、その値を変数$orderに代入
 if(isset($_GET['order'])){
  $order=$_GET['order'];
 }else{
  $order=NULL;
 ...
 //keywordパラメータの値が存在すれば、その値を変数$keywordに代入
...
//SQL文のlike句で使うため、変数＄keywordの前後を％で囲む
...
<body>
  ...
  <main>
    <article class="books">
      <h1>書籍一覧</h1>
      <?php
      //登録・編集・削除messageパラメータの値を受け取っていれば、それを表示
      ...
      <table class="books-table">
        ...
        foreach($books as$book){
          ...
          <td><a href='update.php?id={$book['id']}'><img src='img/edit.png' alt='編集' class='edit-icon'></a></td>
